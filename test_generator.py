...
def run_tests():
    ...
    # Test 4: Involute & Herringbone Profile Geometry Points
    print("\n[TEST 4] Tooth Profile Coordinates Generation")
    pts_ext = ToothProfileGenerator.generate_external_gear_profile_points(z=16, module=1.0, backlash=0.2)
    print(f"  [OK] External Sun/Planet profile generated: {len(pts_ext)} polygon points")
    assert len(pts_ext) > 50, "External points too sparse!"

    pts_int, housing_r = ToothProfileGenerator.generate_internal_ring_gear_profile_points(z_ring=48, module=1.0, housing_outer_dia=60.0, backlash=0.2)
    print(f"  [OK] Internal Ring gear profile generated: {len(pts_int)} polygon points, housing outer radius: {housing_r}mm")
    assert len(pts_int) > 100, "Internal ring points too sparse!"
